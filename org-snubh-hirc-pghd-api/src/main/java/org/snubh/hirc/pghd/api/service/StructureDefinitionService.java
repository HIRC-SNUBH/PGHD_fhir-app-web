...
public class StructureDefinitionService extends BaseService implements IService {
	...
	public Resource readService(IdType idType) throws InstantiationException, IllegalAccessException, ParseException, ConfigurationException, DataFormatException, IOException {
		...
		String path = String.format("resource/structuredefinition/%s.json", idType.getIdPart());
		ClassPathResource classPathResource = new ClassPathResource(path);
		try {
			resource = (StructureDefinition) parser.parseResource(classPathResource.getInputStream());
